...
public class BiomeWarpedForest extends Biome implements INetherBiome, INetherAPIRegistryListener, IAmbienceBiome {
            ...
            for(int y = NetherAPIConfig.tallNether ? 240 : 110; y > 32; y--) {
                ...
                if(depthSignature == 1) {
                    ...
                    if(randTreeSize == 1) { //small
                        ...
                        if(!large_tree.generate(world, rand, pos.add(l6 - 2, y + 2, k10 - 1))) {
                            tree.generate(world, rand, pos.add(l6 -1, y + 2, k10 - 1));
                        }
                    }
                }
    ...
    {
        if (random.nextInt(particleRate) == 0) return new IParticleFactory[] {new ParticlePixel.SporeWarpedFactory()} ;
        else return null;
    }

    @Override
    public ISoundAmbience getRandomAmbientSound() {
        return new DarkSoundAmbience(ModSoundHandler.WARPED_FOREST_AMBIENT, 0.001, 12, 6);
    }
